...
HiveCode uses `settings.json` files for persistent configuration. There are
three locations for these files:

- **User settings file:**
  - **Location:** `~/.gemini/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all HiveCode sessions for the current user.
...
- **System settings file:**
  ...
  - **Scope:** Applies to all HiveCode sessions on the system, for all users.
    ...
    administrators at enterprises to have controls over users' HiveCode setups.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for HiveCode.
  - **Default:** `"Default"`
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for HiveCode. For
    more information, see [Telemetry](./telemetry.md).
  - **Default:**
